/**
  ...
  * @notice 314 byte EIP712 Signing Compliant Delegate-Call Enabled MultiSignature Wallet for the Ethereum Virtual Machine
  */
object "MultiSignatureWallet" {
  code {
    // constructor: uint256(signatures required) + address[] signatories (bytes32 sep|chunks|data...)
    codecopy(0, 314, codesize()) // setup constructor args: mem positon 0 | code size 280 (before args)
  ...
  object "Runtime" {
    code {
        ...
        mstore(64, keccak256(128, 192)) // EIP712 Execute() Type Hash
        ...
        sstore(add(address(), 1), add(1, nonce)) // increase nonce: nonce = nonce + 1
